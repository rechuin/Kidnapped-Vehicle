...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
  ...
  for(i=0; i<num_particles; i++)
  {
    ...
    for(j=0; j<observations.size(); j++)
    {
      ...
      transf_obs.x = observations[j].x * cos(particles[i].theta) - observations[j].y * sin(particles[i].theta) + particles[i].x;  
      transf_obs.y = observations[j].x * sin(particles[i].theta) + observations[j].y * cos(particles[i].theta) + particles[i].y;
    ...
    //std::cout << "total_weight is : " << total_weight << std::endl;
    /* */
  ...
  //double total_weight = 0.0;
  // calculate the total weight of all pariticles
  ...
  {
    particles[q].weight /= total_weight;
